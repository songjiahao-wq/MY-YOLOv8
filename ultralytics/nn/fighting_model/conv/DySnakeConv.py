...
def get_coordinate_map_2D(
    ...
    if morph == 0:
        ...
        y_offset_ = einops.rearrange(y_offset_, "b k w h -> k b w h").cuda()
...
class DySnakeConv2(nn.Module):
    def __init__(self, inc, ouc, k=3, act=True) -> None:
        super().__init__()

        self.conv_0 = Conv(inc, ouc, k)
        self.conv_x = DSConv_pro(inc, ouc, morph=0)
        self.conv_y = DSConv_pro(inc, ouc, morph=1)
        self.conv_1x1 = Conv(ouc * 3, ouc, 1, p=0)
